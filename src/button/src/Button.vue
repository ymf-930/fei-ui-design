<template>
  <button
    ...
    class="fei-button"
    :class="{
      [`fei-button--${btnType}`]: btnType,
      [`fei-button--${size}`]: size,
      [`fei-button--text`]: isNormalTextColor || type === 'text',
  ...
  >
    <span v-if="icon || loading" class="fei-button__icon">
      <i
        v-if="loading"
        class="fei-button-loading icon-is-rotating"
        :class="[
          'f-iconfont',
          `f-icon-${loadingIcon || 'loading'}`,
      ...
      <i
        v-if="icon && !loading"
        :class="['f-iconfont', 'f-icon-' + icon]"
        :style="{ ...textStyle, ...iconStyle }"
      />
    </span>
    <span v-if="$slots.default" class="fei-button__content" :style="textStyle">
      <slot />
    </span>
  </button>
</template>
