...
export default defineComponent({
  ...
  setup(props) {
    const FConfigProvider = inject(configProviderInjectionKey, null)

    // 主题名称注入
    const mergedThemeNameRef = computed(() => {
      const { themeName } = props
      if (themeName === null) return undefined
      return themeName === undefined ? FConfigProvider?.mergedThemeNameRef.value : themeName
    })
    ...
    const mergedThemeRef = computed(() => {
      ...
      const inheritedTheme = deepCopy(FConfigProvider?.mergedThemeRef.value)
    ...
    provide(configProviderInjectionKey, {
      ...
      mergedLocaleRef: computed(() => {
        ...
        return locale === undefined ? FConfigProvider?.mergedLocaleRef.value : locale
      }),
      mergedThemeRef,
      mergedThemeNameRef
    })
