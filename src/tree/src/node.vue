      ...
      >
        ...
        <template v-else>
          ...
          <span v-else :class="titleClasses" :title="data[titleKey]" @click="handleSelect">
            <i v-if="data.icon" :class="`f-iconfont f-icon-${data.icon}`"></i>
            {{ data[titleKey] }}
          </span>
        </template>
      </div>
      <FCollapseTransition>
        <div v-show="data.expand" class="fei-tree-node-children">
          <tree-node
...
export default defineComponent({
  name: 'TreeNode',
  components: { FCollapseTransition, Render, FCheckbox },
  props: {
    data: {
      type: Object,
      default: () => {
